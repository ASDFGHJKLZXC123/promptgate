...
11. **Judge: deterministic assertions first; Phase 5 targets DeepSeek V4 Flash and uses GPT-5.6 Terra only as its independent judge.** Terra judges only DeepSeek target output, so no model judges itself. DeepSeek target calls use temperature zero; Terra judge calls omit the temperature field and retain the immutable `judge_rubric_v1@1` prompt, JSON-object output, high reasoning effort, and no cache. The runtime judge remains a small, explicitly invoked part of the gate and is not used when deterministic assertions are sufficient. The human-approved 2026-07-28 comparability amendment supersedes the earlier Gemini-judge persisted-baseline path for the live Phase 5 gate: the good Verify must freshly pair `safety_screen@prod` and `safety_screen@candidate` under the same Terra judge and must not use `--baseline-from-history`, because persisted eval rows do not identify their judge and baseline run ID 1 was Gemini-judged. General hardened history support remains available outside this active gate. The later owner-approved Terra request-contract amendment changes only that model's temperature omission; it neither removes Gemini, OpenAI, or Anthropic gateway support nor changes Phase 6's credential requirement. The human-approved 2026-07-29 Phase 6 clarification requires dedicated provider-side-capped CI credentials for all four supported providers while retaining the same fresh paired DeepSeek-target/Terra-judge quality gate; Anthropic and Gemini credentials receive no Phase 6 eval traffic and do not count as live adapter verification. The owner-approved 2026-07-29 same-version pairing amendment governs a fresh pair whose frozen baseline and candidate refs resolve to the same prompt id and version: the runner executes and persists exactly one run under the candidate ref, applies the normal dataset threshold, defines the score drop as zero, and prints an explicit line naming both refs and the shared version. Distinct resolved versions retain baseline-first two-run pairing with the 0.05 drop band, and historical comparison is unchanged. The owner-approved 2026-07-30 deterministic negative-control amendment retires certified degraded v2 as the Phase 6 Verify red arm — live it passed 42/50 (0.84) and 45/50 (0.90) and never tripped the 0.8 pass-rate floor — and replaces it, on the disposable never-merged Verify branch only, with an uncertified fixed-output v3 that pins the exact none JSON and withholds the case note, making the expected red deterministic at 16/50 = 0.32 with 34 named risk-label failures and exit 1 through the threshold; any constant output passes at most 24/50, a live result ≥40/50 falsifies the control and requires redesign rather than a rerun, and prod at certified v1, the certified v1/v2 artifacts, dataset, judge topology, threshold, band, and workflow command remain unchanged. After explicitly approving Phase 6 completion, the owner selected post-phase D1 Remedy A: fresh distinct-version and historical comparisons reject a pass-rate drop materially greater than 0.05 in addition to the existing absolute threshold and score-drop checks; exact 0.05 is accepted, fresh same-version pairs define both relative drops as zero, and deterministic failures retain no synthetic rubric score.
