...
function readSeededRows(dbPath: string) {
	...
	try {
		return db
			.prepare(
				"SELECT provider, model, input_micro_usd_per_mtok, output_micro_usd_per_mtok, effective_from FROM model_pricing ORDER BY model, effective_from",
			)
...
function rowsAsSet(rows: readonly SeededPricingRow[]) {
	return new Set(
		rows.map(
			(row) =>
				`${row.provider}|${row.model}|${row.effective_from}|${row.input_micro_usd_per_mtok}|${row.output_micro_usd_per_mtok}`,
		),
	);
}
