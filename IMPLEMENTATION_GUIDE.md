...
- **Tool calls: resolved.** The finished `web_builder_llm` source has a separate native Anthropic path that can send a function tool, so Phase 2 translated supported OpenAI function tools and `tool_choice` to Anthropic equivalents; malformed or unsupported forms fail with a safe 400. Its Phase 8 PromptGate-facing `OPENAI_CHAT` path sends no tools.
...
Also implement `GET /v1/models` (list from currently effective `model_pricing`) — many OpenAI-compat clients call it on startup. The list describes routed/priced capability, not credential readiness: a model can appear while its optional provider key is absent, then fail safely only when called. Phase 8 resolved the external behavior: `web_builder_llm` calls this endpoint conditionally during OpenAI-compatible key validation and again through its live-model lookup after successful validation; it does not call it merely on startup, and a failed/empty listing falls back to configured defaults.
...
- `--min-request-interval-ms` is an opt-in, non-negative safe-integer delay (default `0`; ambient environment values cannot enable it). It applies one shared start-time queue per model across all target and judge calls in each paired command; it neither retries failures nor changes caching, budgets, datasets, models, or exit codes. Use `15000` for the DeepSeek-led gate. Each paired command can invoke up to fourteen Terra rubric judgments: seven for the fresh baseline and seven for the candidate; a same-version pair runs once and can invoke at most seven.
...
- Baseline comparison for the active Phase 5 gate is **freshly paired**: `--baseline prod` runs the baseline ref first, then the candidate, and compares only scores produced under the same Terra judge. Do **not** pass `--baseline-from-history` in either Phase 5 Verify command. Baseline run ID 1 remains valid evidence of the earlier Gemini-judged attempt, but it is not score-comparable with Terra output because `eval_runs` does not persist judge identity. The hardened historical lookup remains a general runner feature with exact dataset/prompt/model/case matching; it is simply not authorized for this gate. This amendment does not change the paired Phase 6 CI sketch. The owner-approved 2026-07-29 same-version pairing amendment governs every fresh pair: when the frozen baseline and candidate refs resolve to the same prompt id and version, the runner executes and persists exactly one run under the candidate ref, applies only the dataset threshold, defines the score drop as zero, and prints an explicit line naming both refs and the shared version; distinct resolved versions keep baseline-first two-run persistence with the 0.05 band, and historical comparison is unchanged.
- Owner-approved post-Phase-6 D1 Remedy A (2026-07-30): every fresh distinct-version and historical baseline comparison also rejects a pass-rate drop materially greater than `--max-pass-rate-drop` (default `0.05`); exact `0.05` is accepted under the same scalar machine-epsilon rail used for score drops. A fresh same-version pair still runs once and defines both score and pass-rate drops as zero. Historical rows must supply valid `cases_passed <= cases_total`; no DB/API/schema change is needed because both fields are already persisted and returned. This leaves `score_avg` judge-only instead of assigning synthetic zero scores to deterministic failures. The active workflow command remains unchanged and relies on the default.
- Exit code contract (this is the CI gate):
  - exit 1 if `pass_rate < threshold` (from dataset `defaultTest.threshold`)
  - exit 1 if `pass_rate` drops more than `--max-pass-rate-drop` (default 0.05) vs a fresh distinct-version or historical baseline run (a same-version fresh pair's drop is zero by definition)
  - exit 1 if `score_avg` drops more than `--max-score-drop` (default 0.05) vs the baseline run (a same-version fresh pair's drop is zero by definition)
...
- Every run writes `eval_runs` + `eval_results` (via the admin API) with `git_sha` (from `GITHUB_SHA` or `git rev-parse`) and `dataset_hash`, and prints a markdown summary table to stdout (becomes the PR comment). Every baseline comparison appends baseline/candidate pass rates, their computed drop, and the allowed limit. Fresh paired runs also append baseline/candidate score averages, both scored counts, and the computed score drop after the per-case table.
...
The owner-approved 2026-07-29 **same-version pairing** amendment keeps the seeded no-op green PR honest: `seed-ci` leaves `prod` and `candidate` on the same certified v1, so the paired command resolves both refs to one version and now runs the candidate exactly once with a zero score drop instead of measuring judge nondeterminism against itself. The first live attempt to complete evaluation (PR #4) supplied the empirical basis: setup, pricing seeding, CI seeding, and cleanup all passed, but the pre-amendment same-version pair returned quality exit 1 with the candidate at 48/50 and score 0.881666667 over six scored cases — one 0.42 rubric disagreement plus the known over-triage case — while the identically versioned baseline's exact score was never emitted and cannot be recovered from the discarded ephemeral database.

The owner-approved 2026-07-30 **deterministic negative-control** amendment governs the red arm of Phase 6 Verify. Certified degraded v2 (`4f9969b7…`) is retired as that negative control: its live runs scored 42/50 (0.84) and 45/50 (0.90) and never tripped the 0.8 pass-rate floor, and its only red arm was the volatile ≤7-case rubric average, which deterministic failures prune scored cases out of and therefore bias upward. On the disposable Verify branch only — never merged, deleted after screenshot — the `safety_screen` fixture carries an uncertified third version whose system message pins the exact output `{"risk_level":"none","guidance":"No action needed."}` and whose user message does not reference `{{note}}` (the `note` variable stays declared; the dataset continues to supply it and the renderer permits unused supplied variables), so the target never sees case content and any constant output passes at most max(24, 10, 16) = 24/50 < 40 = 0.8·50. `candidate` points at v3, `prod` stays at certified v1, and the v3 sha256 is pinned beside the two byte-unchanged certified digests in `ci-seed.ts` (`NEGATIVE_CONTROL_DIGEST`). The fixture-validator widening to three versions (tuple, candidate-label union, `addSafetyVersion` 1|2|3, `ensureSafetyPrompt` bounds) is Zod input validation at the checked-in-fixture trust boundary, not the protected DB/API/persistence schema. The expected fixed result from the frozen 24/10/16 label distribution is 16/50 = 0.32 pass rate, 34 named risk-label failures, exit 1 through the dataset threshold; ≥40/50 live invalidates the control and requires redesign with no rerun. The disposable branch may update only the `ci-seed` test expectations required to keep ordinary `ci` green; the workflow command, dataset/hash, threshold, band, models, pacing, cache/retry/self-judge, persistence, exit codes, budget, and credentials are unchanged.

The project owner explicitly approved Phase 6 completion on 2026-07-30, then selected **D1 Remedy A** with a 0.05 pass-rate drop limit. The §7.2 comparison now rejects deterministic degradation relative to either a fresh distinct-version or historical baseline even when the candidate's surviving rubric cases raise `score_avg`. Same-version fresh pairs still run once with zero relative drops, the absolute 0.8 dataset threshold remains, and the CI command continues to rely on defaults. This is a post-Phase-6 runner correction; it does not alter or rerun the captured green/red Verify evidence.
...
governs this plan: do not describe the finished app's buffered OpenAI-compatible
client as token streaming, and do not widen the external application's
transport merely to satisfy the original shorthand.

1. Run PromptGate via docker compose on the same host as web_builder_llm.
2. Create key `web_builder_llm` (budget e.g. $5/mo).
3. Point web_builder_llm at PromptGate with the resolved app-level provider JSON: `providerId=promptgate`, `apiFamily=OPENAI_CHAT`, Bearer auth, `baseUrl=http://127.0.0.1:8787/v1`, `defaultEnvKey=PROMPTGATE_API_KEY`, `jsonStrategy=NATIVE_JSON_OBJECT`, and default model `deepseek-v4-flash`; select it through `LLM_PROVIDERS_CONFIG` or `site.llm.providers.config` and explicitly set `site.llm.allowPrivateBaseUrls=true` for trusted loopback. Key validation and live model discovery call `GET /v1/models`; this path sends no tools.
4. Verify normal generate/edit flows through PromptGate. The current provider completion is buffered and the app's SSE replays progress after synchronous completion; confirm that the existing UX is unchanged and durable request rows accrue, without claiming live provider-token streaming. The scheduled provider contracts in §11 own token-stream proof.
5. Migrate one app prompt into the registry, switch that call site to `pg_prompt`, and prove a live attributed call before opening the observation window.
6. Retain at least seven consecutive calendar days with the registry-backed path live and at least one real request on five of those seven days. The closing Overview + Quality Drift screenshots go in the README case study. State that the client is buffered; calculate cache dollars saved as the sum of retained `cache_saved_micro_usd` over known cache-hit rows, with exact and estimated sums separate and the count of excluded unknown legacy hits reported separately; calculate gateway p95 overhead as proxied p95 minus direct p95 on the same model, prompt, sample size, and window with cache hits excluded; and prove a registry-label rollback changed the next dogfood call/version.

Definition of "dogfood proven": the registry-backed call is live before a
seven-consecutive-calendar-day observation window; at least five of those seven
days contain a real dogfood request; normal buffered generate/edit behavior and
durable attribution are evidenced; and a registry-label rollback changes the
next dogfood request's served version.
...
- **Contract (nightly, live, tiny):** `contract-nightly.yml` (created in playbook phase 8, step 6 — explicitly deferred until then) sends one minimal streaming + one non-streaming request per provider with pinned cheap models and asserts response shape against the adapter's Zod schemas. A missing credential produces a named `SKIPPED` provider result and never live-green; a configured provider's contract failure makes the workflow red; and zero configured providers makes the workflow red. Phase 8 Verify retains at least one green schedule-triggered run rather than relying only on manual dispatch. Drift → red badge before it breaks a workday. This closes the idea file's "contract testing against provider APIs" bonus concept.
...
| CI eval flakiness (LLM nondeterminism) fails good PRs | deterministic assertions dominate the gate; judge cases use threshold + `max-score-drop` band, not exact match; deterministic regression also has the independent `max-pass-rate-drop` band so failed cases cannot improve the candidate by disappearing from `score_avg`; Phase 5 pins DeepSeek V4 Flash at temperature zero and uses GPT-5.6 Terra at high reasoning effort without a temperature override as its independent judge instead of self-judging; a fresh pair resolving to one identical prompt version runs once with zero score and pass-rate drops, so an unchanged prompt cannot fail its own judge-nondeterminism comparison (the first completed Phase 6 live pair, PR #4, failed exactly that way) |
...
| web_builder_llm tool behavior blocks dogfood | resolved from the finished source: the PromptGate-facing OpenAI Chat path sends no tools, while Phase 2 already translated supported tools for the separate Anthropic-routed path |
...
Open a terminal and execute Phase 0, step 1 of `BUILD_PLAYBOOK.md`. Every subsequent action is the next numbered step; every phase ends with a verify block that tells you whether to proceed. Both external-repository `TODO(verify)` items are resolved and recorded in `PROGRESS.md`; only explicitly marked `TODO(build-time)` checks for current model names, prices, or provider contracts remain time-sensitive.
