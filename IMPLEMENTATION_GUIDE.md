...
- Phase 5 target and judge requests use `temperature: 0`. A workable rotated `OPENAI_API_KEY` is therefore required for the live Terra judge, while `DEEPSEEK_API_KEY` remains required for the target. Gemini remains a supported gateway provider but is no longer part of the active Phase 5 target/judge topology. This amendment does not waive Phase 6's separate four-provider requirement.
