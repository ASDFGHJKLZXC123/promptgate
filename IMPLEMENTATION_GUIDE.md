...
- Every run writes `eval_runs` + `eval_results` (via the admin API) with `git_sha` (from `GITHUB_SHA` or `git rev-parse`) and `dataset_hash`, and prints a markdown summary table to stdout (becomes the PR comment). Fresh paired runs append baseline/candidate score averages, both scored counts, and the computed score drop after the per-case table.
