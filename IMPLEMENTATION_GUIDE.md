...
./node_modules/.bin/pg-eval run \
  ...
  --baseline prod --baseline-from-history --gateway http://localhost:8787 \
...
- `--min-request-interval-ms` is an opt-in, non-negative safe-integer delay (default `0`; ambient environment values cannot enable it). It applies one shared start-time queue per model across all target and judge calls; it neither retries failures nor changes caching, budgets, datasets, models, or exit codes. Use `15000` for the DeepSeek-led gate. Under the approved persisted-baseline path, run the remaining good and deliberately degraded commands on separate fresh Gemini daily-quota days; each candidate-only command can consume at most seven Gemini rubric calls.
...
- Baseline comparison remains **paired by default**: `--baseline prod` runs the baseline ref itself first (same dataset, same models, same session), then the candidate, and compares within the pair — so it works in a fresh database and remains the Phase 6 CI contract. The human-approved Phase 5 live quota exception accepts completed baseline run ID 1 from code HEAD `a77b9cf` as the current baseline evidence. The two remaining Phase 5 Verify commands must pass `--baseline prod --baseline-from-history`; before dataset upsert, provider traffic, or any other admin mutation, the runner must validate a persisted row whose dataset slug and exact frozen dataset hash, baseline prompt ID/ref/version, target model, and case count all equal the current loaded gate. A malformed history response or no exact row is a sanitized infrastructure exit 2 with no provider/admin mutation side effects. Only the new candidate is executed and persisted; the stopped, unpersisted partial candidate is never eligible for reuse. This exception does not change the paired Phase 6 CI sketch or general historical-run support.
