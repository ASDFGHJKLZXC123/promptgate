...
The human-approved 2026-07-28 Phase 5 persisted-baseline quota amendment accepts completed baseline run ID 1 from code HEAD `a77b9cf` as the current live baseline evidence. Each remaining good/degraded Verify command must add `--baseline-from-history` while retaining `--baseline prod` and `--min-request-interval-ms 15000`. Before dataset upsert, provider traffic, or any admin mutation, history acceptance must exactly match the current dataset slug/hash and case count, frozen baseline prompt ID/ref/version, and target model; malformed, missing, or mismatched history is sanitized infrastructure exit 2 with no provider/admin mutation side effects. Never reuse the stopped, unpersisted partial candidate. Run the two remaining candidate commands on separate fresh quota days. Preserve all 50 cases, seven rubrics, DeepSeek/Gemini topology, no retries/cache/self-judge, $1 keys, persistence/table/exit/direct-bin contracts, general provider support, and Phase 6's paired four-provider gate.
