...
- **Do not use a Haiku model anywhere in this project.** The human-approved 2026-07-28 decision #11 amendment makes GPT-5.6 Terra the independent high-effort judge of DeepSeek V4 Flash's sole Phase 5 target output.
...
| Phase 5 runtime LLM-as-judge (in-product) | GPT-5.6 Terra / high judges DeepSeek V4 Flash | Amended decision #11; deterministic assertions run first, Terra judges only DeepSeek target output, and neither model judges itself. |
...
The human-approved 2026-07-25 provider amendment permits an explicit **available-key live matrix** in phase verify blocks: every implemented provider whose key is configured must be called; every missing-key activation check must be named as deferred with its reason and must never be described as live-green. Offline parity tests and all non-live acceptance criteria still apply to every approved provider. The human-approved 2026-07-28 decision #11 amendment makes DeepSeek the exact sole Phase 5 target and GPT-5.6 Terra its exact independent high-effort judge; it does not waive CI requirements in phase 6.

The Phase 5 request pace remains opt-in: `--min-request-interval-ms 15000` shares a per-model request-start queue across all target and judge calls in each paired run. Its default is `0`, and ambient environment values cannot enable it; it does not authorize retries or alter cache, budget, dataset, model, or exit-code behavior.

The human-approved 2026-07-28 Phase 5 Terra-judge comparability amendment supersedes the earlier persisted-baseline exception for this gate. Baseline run ID 1 was judged by Gemini, and current `eval_runs` rows do not persist judge identity, so neither literal Phase 5 command may pass `--baseline-from-history`. Each must freshly pair `--baseline prod` and the candidate under Terra with `reasoning_effort: "high"`, no temperature field, JSON-object output, and `--min-request-interval-ms 15000`; the DeepSeek target retains temperature zero. Preserve hardened general history support, all 50 cases, seven rubrics, no retries/cache/self-judge, $1 keys, persistence/table/exit/direct-bin contracts, general provider support, and Phase 6's paired gate plus four capped-credential requirement.

The human-approved 2026-07-28 Phase 5 SQLite lifecycle amendment is a hard pre-paid rail. Retain WAL and the existing bind mount; SIGTERM/SIGINT must share one awaited Fastify shutdown; the close hook must validate a successful TRUNCATE checkpoint and close the database even on checkpoint failure; Compose stop grace must exceed the upstream timeout; and host `sqlite3` is forbidden while the gateway is live. Before another provider call, pass the production-sized 50-result persistence/reopen regression, signal/main-file regression, complete offline gate, and a no-provider Docker bind-mount write → graceful stop → host read → restart/API read canary on the exact committed image. The score-average comparator may admit only scored-count-scaled IEEE-754 addition-order roundoff; nullability, counts, pass totals, and micro-USD totals stay exact. If any durability gate is red, stop before paid traffic. The owner authorized exactly one new fresh good Verify after all gates are green; key 22 and its lost sequence are never reusable. The degraded run, checkpoint B1/B2, and Phase 6 remain ordered afterward.

The human-approved 2026-07-29 Phase 6 Recovery A contract is a hard fresh-run rail. `seed-ci` must reserve an empty mode-`0600` handoff before its one-time key POST; initial creation permits zero gateway keys and intact replay permits exactly one enabled `ci-evals` key with the exact locked budget/rate whose saved plaintext authenticates. Any other inventory fails before prompt/dataset mutation. An interrupted or ambiguous POST-to-durable-handoff sequence invalidates the whole ephemeral gateway/database/handoff, which must be discarded and recreated before retry; do not recover it by rotating keys or changing the API, DAO, migration, or schema.

The human-approved 2026-07-30 post-Phase-6 D1 Remedy A is a hard eval-runner rail: fresh distinct-version and historical baseline comparisons reject a candidate pass-rate drop materially greater than `--max-pass-rate-drop` (default `0.05`) in addition to the absolute dataset threshold and score-drop check. Exact `0.05` is accepted under the scalar machine-epsilon comparator; fresh same-version pairs run once and define both relative drops as zero. Keep deterministic failures unscored by the rubric, require valid historical `cases_passed`, and leave the Phase 6 workflow invocation unchanged. Publish this correction before starting Phase 7 implementation.
