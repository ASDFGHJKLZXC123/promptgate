...
4. **Gateway client.** Thin fetch wrapper (base URL + key from flags/env). Phase 5 DeepSeek target calls use `temperature: 0`; Terra judge calls omit temperature and send `reasoning_effort: "high"`. All calls set **`pg_no_cache: true`** (persisted quality runs must hit live models — cached responses conceal provider drift; `--allow-cache` is for local harness development only) and `pg_feature: "eval"`.

5. **Judge.** `llmRubric` calls the gateway through the locked map: **DeepSeek target → Terra judge**. It uses registry prompt `judge_rubric_v1@1` (create it via seed script — the rubric prompt is itself versioned, per §7.2), `reasoning_effort: "high"`, `response_format: {type: "json_object"}`, and parses `{pass, score, rationale}`. Reject every unapproved target model before admin mutation or provider traffic, and reject any self-judge mapping. Malformed judge output = infrastructure error (exit 2), not a case failure.

6. **Run + persist + compare.** At start, resolve every label ref to a concrete version (frozen for the whole run, §7.2), then upsert the dataset via `POST /admin/api/evals/datasets`. For the active Phase 5 gate, `--baseline prod` is strictly **paired**: run the baseline ref first, then the candidate, persist one target-model run plus results for each through `POST /admin/api/evals/runs`, and compare only within that same-judge pair (admin token from `--admin-token`/`PG_ADMIN_TOKEN` — eval traffic and persistence use different credentials, §5.2). Do not pass `--baseline-from-history`: run ID 1 was judged by Gemini and `eval_runs` does not persist judge identity, so its score cannot be compared with a Terra-judged candidate. Keep the hardened historical lookup and its exact identity checks as a general feature; do not use it for either literal Phase 5 command. `--min-request-interval-ms` remains an opt-in per-model request-start pace (default `0`) shared by target and judge calls; use `15000` ms for the DeepSeek-led gate. Each paired command can consume at most fourteen Terra rubric calls. Do not add retries or change cache/budget, dataset/model, persistence/table, exit-code, direct-bin, or $1-key contracts. Owner-approved 2026-07-29 same-version pairing: when the frozen baseline and candidate refs resolve to the same prompt id and version, run and persist the candidate exactly once, apply only the dataset threshold, define the score drop as zero, and print one explicit line naming both refs and the shared version; distinct resolved versions keep baseline-first two-run persistence with the 0.05 band, and history mode is unchanged. Apply §7.2's exit-code contract verbatim and print the markdown summary table (case, model, pass, score, first failed assertion detail), followed for fresh pairs by baseline/candidate score averages, both scored counts, and the computed drop. The Phase 6 workflow command remains unchanged.

Owner-approved post-Phase-6 D1 Remedy A (2026-07-30): every baseline comparison also rejects a candidate pass-rate drop materially greater than `--max-pass-rate-drop` (default `0.05`). Use the same scalar machine-epsilon boundary as score-drop comparison; exact `0.05` is accepted. Fresh same-version pairs still run once and define both score and pass-rate drops as zero. Historical comparison now requires the persisted `cases_passed` count and applies the same pass-rate band; its score behavior remains unchanged. Print baseline/candidate pass rates, their drop, and the allowed limit so a pass-rate-only quality exit 1 is explicit. The active Phase 5 and Phase 6 commands rely on the default and remain byte-unchanged.
...
# Stop here if the fresh Terra-judged pair does not return 0; preserve its evidence.
...
Owner-approved same-version pairing amendment (2026-07-29): the seeded gateway leaves `prod` and `candidate` at the same certified v1, so a green-path PR's fresh pair resolves both refs to one version; the runner executes and persists that candidate exactly once, applies the 0.8 dataset threshold, defines the score drop as zero, and prints one explicit line naming both refs and the shared version, plus baseline/candidate score averages, both scored counts, and the computed drop for every fresh pair. Distinct resolved versions — the degraded PR — keep baseline-first two-run persistence with the 0.05 band; history mode, dataset/hash, threshold, models, pacing, cache/retry/self-judge rules, exit codes, schema, budget, credentials, and the workflow command are unchanged. Empirical basis: the first live attempt to complete evaluation (PR #4) passed setup, pricing seeding, CI seeding, and cleanup, but the pre-amendment same-version pair returned quality exit 1 — candidate 48/50 with score 0.881666667 over six scored cases, one 0.42 rubric disagreement and the known over-triage case — while the baseline's exact score was never emitted and cannot be recovered from the discarded ephemeral database.

Owner-approved deterministic negative-control amendment (2026-07-30): certified degraded v2 (`4f9969b7…`) is retired as the Phase 6 Verify negative control — its failure count is set by the target model's judgment rather than the prompt: live runs scored 42/50 (0.84) and 45/50 (0.90), never tripping the 0.8 pass-rate floor, and its only red arm was the volatile ≤7-case rubric average, which deterministic failures bias upward by pruning scored cases. On the disposable Verify branch only — never merged, deleted after screenshot — the checked-in fixture gains an uncertified third version whose system message pins the exact output `{"risk_level":"none","guidance":"No action needed."}` and whose user message does not reference `{{note}}` (the variable stays declared; the dataset still supplies it and the renderer permits unused supplied variables), so the model never sees case content and no instruction-ignoring behavior can reproduce expected labels; `candidate` points at version 3, `prod` stays at certified v1, and the v3 sha256 is pinned in `ci-seed.ts` beside the two byte-unchanged certified digests. The fixture-validator widening (tuple, candidate-label union, version bounds 2→3) is input validation at a checked-in-JSON trust boundary, not the protected DB/API/persistence schema. The expected outcome is fixed offline by the test-pinned 24/10/16 label distribution: pass rate 16/50 = 0.32, 34 named `risk-label.js` failures, exit 1 via the dataset threshold alone; any constant output passes at most 24/50 < 40, so no reachable model behavior attains 0.8. The disposable branch may update only the `ci-seed` test expectations required to keep ordinary `ci` green, so the red is attributable to `eval-gate` alone. All other approved invariants — dataset/hash, threshold 0.8, band 0.05, models, pacing, cache/retry/self-judge rules, exit codes, persistence, budget, credentials, and the one-invocation baseline-first workflow command — are unchanged. The prediction is pre-registered in `PROGRESS.md` before pushing; a live pass rate ≥ 0.8 (≥40/50) invalidates the control and requires redesign, not a rerun.

Phase 6 completion and D1 disposition (owner-approved 2026-07-30): Phase 6 is complete and human-approved on the evidence in `docs/evidence/phase-6.md`. The separately approved Remedy A closes defect D1 with the §7.2 pass-rate drop band instead of inserting artificial zero rubric scores for deterministic failures. This post-phase correction changes no dataset threshold, prompt fixture, target/judge topology, workflow invocation, persistence schema, provider credential, budget, or captured Phase 6 Verify result; publish it before Phase 7 starts.

1. **Seed script.** `pg-eval seed-ci` (runs against a fresh gateway): create key `ci-evals` with `budget_micro_usd_month: 1000000` ($1 — §7.4's circuit breaker, enforced by reserve-then-reconcile), create `safety_screen` + `judge_rubric_v1` prompts from checked-in JSON fixtures (`packages/evals/fixtures/prompts/*.json`), set labels `prod` and `candidate`, register the dataset. Idempotent within one intact gateway/handoff pair. **Human-approved Recovery A (2026-07-29):** before the one-time key POST, reserve an empty mode-`0600` `PG_EVAL_KEY_FILE` handoff. Initial seed requires zero gateway keys; reuse requires exactly one enabled key, named `ci-evals`, with the exact locked budget/rate and a saved plaintext that authenticates. Any other key inventory fails before prompt or dataset mutation. If key creation is interrupted or ambiguous before the handoff is durably written, fail closed and discard/recreate the entire ephemeral gateway, database, and handoff before rerunning; do not add key rotation or widen the API/schema.

2. **Secrets.** Create **dedicated CI keys at each provider with provider-side spend limits** (the gateway's $1 budget can't stop PR code from calling providers directly with the env keys) and add them as `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GEMINI_API_KEY`, and `DEEPSEEK_API_KEY` repo secrets. The human-approved Phase 6 interpretation requires all four capped credentials but retains the fresh paired `deepseek-v4-flash` target → `gpt-5.6-terra` judge quality gate; Anthropic and Gemini receive no Phase 6 eval traffic and their configured presence is not live adapter verification. Any credential omission or target/judge expansion needs a separate human gate amendment. `ADMIN_TOKEN` for CI is generated per-run — the workflow writes it and the provider keys into `.env` so **docker compose actually passes them to the gateway container** (secrets set only on a step's `env:` never reach the container).

3. **Workflow.** Commit §7.4's `eval-gate.yml`, replacing every `<pinned-sha>` with the action's full commit SHA (supply-chain pin). Before starting the fresh gateway, run the built image's checked-in pricing seeder against the mounted ephemeral database; migrations create an empty `model_pricing` table and the live target must never be dispatched without an approved price. **Human-approved stable-check amendment (2026-07-29):** trigger on every `pull_request` with no event-level `paths`, then use a no-secret detector job over the complete merge-base-to-head pull-request diff. Relevant paths are `packages/gateway/**`, `packages/evals/**`, `packages/shared/**`, `Dockerfile`, `docker-compose.yml`, `.dockerignore`, `.nvmrc`, `package.json`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`, `tsconfig.base.json`, `.github/workflows/eval-gate.yml`, `.github/workflows/ci.yml`, and `.github/actions/**`; an unresolved diff is relevant. Only a relevant trusted same-repository PR may run the secret-bearing live job. An always-running aggregate job named `eval-gate` returns green for an irrelevant no-op or a passing live gate and red for detector failure, a relevant untrusted source, or live-gate failure. Never use `pull_request_target`. The `comment` subcommand reads the summary markdown and posts via `GITHUB_TOKEN` — optional; skip if you'd rather read logs.

4. **Branch protection.** GitHub → Settings → Branches → protect the repository default branch (currently `master`; do not rename it for this phase) and require the stable `eval-gate` and `ci` job checks. Apply the rule to administrators, require the branch to be up to date, and do not add an unrequested review requirement.

**Verify phase 6:** open two PRs: one no-op (green), one that re-points `candidate` at the uncertified deterministic negative-control v3 (2026-07-30 amendment above; certified degraded v2 is retired from this role because it passed live at 0.84 and 0.90) — the check must go red via the pass-rate floor and the log/comment must name the failing cases. Merge neither; delete after screenshot (the red one is README material).
